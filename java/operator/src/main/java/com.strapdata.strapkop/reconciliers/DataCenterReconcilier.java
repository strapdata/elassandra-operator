package com.strapdata.strapkop.reconciliers;
...
public class DataCenterReconcilier {
    ...
    private final BehaviorSubject<Tuple2<Action, DataCenter>> queue;
    private final ApplicationContext context;
    
    public DataCenterReconcilier(ApplicationContext context) {
        this.context = context;
        queue = BehaviorSubject.create();
        queue.subscribeOn(Schedulers.single())
                .doOnError(throwable -> {
