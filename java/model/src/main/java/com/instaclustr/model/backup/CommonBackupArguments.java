package com.instaclustr.model.backup;
...
public abstract class CommonBackupArguments extends BaseArguments {
    ...
    void printHelp() {
        ...
        stream.format("Valid time units are: %s%n", Arrays.stream(TimeUnit.values()).map(Enum<TimeUnit>::name).map(String::toLowerCase).collect(Collectors.toList()));
        stream.format("Valid data rate units are: %s%n%n", Arrays.stream(DataRate.DataRateUnit.values()).map(Enum<DataRate.DataRateUnit>::name).map(String::toLowerCase).collect(Collectors.toList()));
    ...
    @Option(name = "-s", aliases = {"--speed"}, usage = "Speed to upload the com.com.backup.", forbids = {"--bandwidth", "--concurrent-connections"})
    @Nullable
    public Speed speed;
