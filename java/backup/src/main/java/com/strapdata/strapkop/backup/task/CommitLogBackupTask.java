...
import com.strapdata.strapkop.backup.uploader.FilesUploader;
import com.strapdata.strapkop.backup.util.Directories;
import com.strapdata.strapkop.backup.util.GlobalLock;
import com.strapdata.strapkop.model.backup.CommitLogBackupArguments;
...
public class CommitLogBackupTask implements Callable<Void> {
    ...
    protected void call0() throws Exception {
        ...
        try (final DirectoryStream<Path> commitLogs = Files.newDirectoryStream(commitLogArchiveDirectory, filter)) {
            ...
            logger.debug("{} files in manifest for commitlog com.com.backup.", manifest.size());
