...
public class AWSSnapshotUploader extends SnapshotUploader {
    ...
    public RemoteObjectReference taskDescriptionRemoteReference(String taskName) throws Exception {
        return new com.strapdata.strapkop.backup.common.AWSRemoteObjectReference(Paths.get(Constants.TASK_DESCRIPTION_DOWNLOAD_DIR).resolve(taskName), resolveTaskDescriptionRemotePath(taskName));
    }
    @Override
